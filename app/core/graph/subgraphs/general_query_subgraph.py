"""
subgraphs/general_query_subgraph.py
-------------------------------------
Stub subgraph for General Query issues on iGOT Karmayogi.

Categories handled (from CATEGORY_SUBCATEGORY_MAP → general):
...
class GeneralQuerySubgraph(BaseSubgraph):
    ...
    def system_prompt(self, state: TicketState) -> str:
        return STUB_SUBGRAPH_SYSTEM_PROMPT.format(
            ...
            main_category=state.get("main_category", "general"),
        )
...
general_query_subgraph = GeneralQuerySubgraph().build()
