"""
subgraphs/virtual_event_subgraph.py
-------------------------------------
Stub subgraph for Virtual Event issues on iGOT Karmayogi.

Categories handled (from CATEGORY_SUBCATEGORY_MAP → virtual_event):
...
class VirtualEventSubgraph(BaseSubgraph):
    ...
    def system_prompt(self, state: TicketState) -> str:
        return STUB_SUBGRAPH_SYSTEM_PROMPT.format(
            ...
            main_category=state.get("main_category", "virtual_event"),
        )
...
virtual_event_subgraph = VirtualEventSubgraph().build()
